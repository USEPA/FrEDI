...
#'
#' As of FrEDI Version 4.1.0, [FrEDI::run_fredi_extremes()] calculates impacts at the state-level for all available sectors.
#'
...
#'
#'
#' @details This function allows users to project annual average impacts from temperature and sea level change through 2300 (2010-2300) for available sectors. [FrEDI::run_fredi_extremes()] is the main function in the [FrEDI] R package, described elsewhere (See <https://epa.gov/cira/FrEDI> for more information).
#'
#' Users can specify an optional list of custom scenarios with `inputsList` (for more information on the format of inputs, see [FrEDI::import_inputs()]). The function [FrEDI::import_inputs()] can be used to importing custom scenarios from CSV files. [FrEDI::import_inputs()] returns a list with named elements `gdp`, `pop`, `temp`, and `slr`, with each containing a data frame with a custom scenario for GDP, state-level population, temperature, and/or GMSL respectively. If a user imports scenarios using [FrEDI::import_inputs()], they can pass the outputs of [FrEDI::import_inputs()] directly to the [FrEDI::run_fredi_extremes()] argument `inputsList`. Note that the documentation for [FrEDI::import_inputs()] can also provide additional guidance and specification on the formats for each scenario type.
#'
#' Otherwise, [FrEDI::run_fredi_extremes()] looks for a list object passed to the argument `inputsList`. Within that list, [FrEDI::run_fredi_extremes()] looks for named list elements -- `gdp`, `pop`, `temp`, and/or `slr` -- each containing a data frame with a custom scenario for GDP, state population, temperature, and/or GMSL, respectively. If `inputsList = NULL` or `inputsList = list()` (default), [FrEDI::run_fredi_extremes()] uses default trajectories for GDP, population, temperature, and SLR (see [FrEDI::gdpDefault], [FrEDI::popDefault], and [FrEDI::gcamScenarios] for more information). [FrEDI::run_fredi_extremes()] will default back to the default scenarios for any list elements that empty or `NULL` (in other words, running `run_fredi_extremes(inputsList = list())` returns the same outputs as running [FrEDI::run_fredi_extremes()]).
#'
...
#'    * If the user does not specify an input scenario for GDP (i.e., `inputsList = list(gdp = NULL)`, [FrEDI::run_fredi_extremes()] uses a default GDP scenario.
...
#'    * If the user does not specify an input scenario for population (i.e., `inputsList = list(pop = NULL)`, [FrEDI::run_fredi_extremes()] uses a default population scenario.
...
#'    * If the user does not specify an input scenario for temperature (i.e., `inputsList = list(temp = NULL)`, [FrEDI::run_fredi_extremes()] uses a default temperature scenario.
...
#'
#' If `outputList = FALSE` (default), [FrEDI::run_fredi_extremes()] returns a data frame of annual average impacts over the analysis period, for each sector, variant, impact type, impact year, region, state, model type (`"GCM"` or `"SLR"`), and model. If `outputList = TRUE`, in addition to the data frame of impacts, [FrEDI::run_fredi_extremes()] returns a list object containing information about values for function arguments, driver scenarios, and population and GDP scenarios.
#'
#'
#'
#' @return
#' If `outputList=FALSE`, the output of [FrEDI::run_fredi_extremes()] is a data frame object (described above) containing annual average impacts over the analysis period, for each sector, variant, impact type, impact year, region, state, and model (GCM name for temperature-driven sectors and `"Interpolation"` for SLR-driven sectors).
#'
#' If `outputList=TRUE`, [FrEDI::run_fredi_extremes()] returns a list object containing the following:
#'
#' * __`statusList`__. A list with values for the arguments passed to [FrEDI::run_fredi_extremes()] (including defaults if unspecified).
#' * __`argsList`__. A list with elements named after [FrEDI::run_fredi_extremes()] arguments, containing the values of the arguments passed to [FrEDI::run_fredi_extremes()] (or default values if unspecified).
...
#'
#' ### Run custom scenario
#' run2 <- run_fredi_extremes(inputsList=list(temp=temps1, pop=popDefault))
#'
...
#' run3 <- run_fredi_extremes(inputsList=x_inputs)
#'
#' ### Get information on sectors:
#' get_sectorInfo()
#'
#' ### Run for a single sector, with default inputs, no aggregation, and elasticity=1:
#' run4 <- run_fredi_extremes(sectorList="ATS Temperature-Related Mortality", aggLevels="none", elasticity=1)
#'
...
#' run5 <- run_fredi_extremes(maxYear=2110)
#'
...
#' run6 <- run_fredi_extremes(thru2300=TRUE)
#'
#'
#'
#'
...
run_fredi_extremes <- function(
    inputsList = list(temp=NULL, slr=NULL), ### List of inputs
  ...
  ### Other values
  co_sectors   <- "co_sectors"    |> get_frediDataObj("frediData", listName="extremesData")
  co_modTypes  <- "co_modelTypes" |> get_frediDataObj("frediData", listName="extremesData")
  ...
  ### Add info to data
  co_inputInfo <- "co_inputInfo" |> get_frediDataObj("frediData", listName="extremesData")
  co_inputInfo <- co_inputInfo |> mutate(ref_year = c(1995, 2000))
  co_inputInfo <- co_inputInfo |> mutate(min_year = c(2000, 2000))
  ...
  inputDefs    <- inNames0 |> map(function(name0){
    ...
    df0      <- defName0 |> get_frediDataObj("scenarioData", listName="fredi")
    ### Format data
  ...
  if(hasAnyInputs) {
    ...
    inputsList <- list(
      ...
      pmap(check_input_data, module="extremes") |>
      set_names(inNames)
    rm(minYrs0, maxYrs0)
  ...
  df_results   <- seScenario |> initialize_resultsDF(sectors=sectorIds, module="extremes") |> ungroup()

  #### Calculate Scaled Impacts --------------
  ### Get scaled impacts
  df_impacts   <- sectorIds |> calc_scaled_impacts_fredi(drivers0=df_drivers, module="extremes") |> ungroup()

  #### Calculate Total Impacts --------------
  ### Get impacts
  df_results   <- df_results |> calc_impacts_fredi(df1=df_impacts, module="extremes") |> ungroup()
  rm(df_impacts)
  ...
  ##### Column Lists
  ###### Grouping Columns
  groupCols0  <- c("sector", "variant", "impactType", "impactYear", "region", "state", "postal")
  ...
  impactCols0 <- c("physical_impacts")
  ##### Columns to select
  ...
  #### Format sectorprimary, includeaggregate --------------
