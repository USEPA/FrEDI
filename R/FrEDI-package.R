...
#'
#'
#' FrEDI is an R package that is developed and maintained by the U.S. Environmental Protection Agency (EPA). The functions and data provided in this package can be used to project economic impacts of risks within the contiguous United States (CONUS) from changes in temperature and sea level, under any custom temperature or socioeconomic trajectory, using the [Framework for Evaluating Damages and Impacts (FrEDI)](https://epa.gov/cira/FrEDI/) that was developed as part of EPA's [CIRA](https://epa.gov/cira/) project. The FrEDI package contains R code that implements FrEDI and allows users to project annual impacts from changes in temperature and sea level across over 20 impact sectors, geographic regions, and populations through the end of the 21st century (and optionally through 2300). While this framework does not currently account for all ways in which the American public may be impacted by future changes in temperature and sea level, this type of detailed information helps EPA to better understand and communicate the types of potential impacts from future risks in the United States, as well as the potential benefits of greenhouse gas mitigation and adaptation.
#'
...
#'
#' The function [FrEDI::run_fredi()] provided in this package is the primary function implementing the [Framework for Evaluating Damages and Impacts (FrEDI)](https://epa.gov/cira/FrEDI/), developed by the U.S. EPA for projecting annual impacts from changes in temperature and sea level. The main inputs to [FrEDI::run_fredi()] are scenarios for temperatures in degrees Celsius, global mean sea level rise in centimeters, U.S. gross domestic product, and state population.
#'
...
#'
#' Versions 2.3.0 and above include the `FrEDI` SV module for performing distributional analyses on estimated impacts from risks arising from changes in temperature and sea level for select sectors. [FrEDI::get_sv_sectorInfo()] allows users to access a list of sectors within the FrEDI SV module and related sector information. The function [FrEDI::run_fredi_sv()] is the main function for the `FrEDI` SV module. [FrEDI::run_fredi_sv()] is designed to calculate impacts for a single sector at a time for a custom population scenario or one or more custom temperature or sea level rise scenarios.
#'
#'
#'
