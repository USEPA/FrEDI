...
load_frediDB <- function(
  ...
  if(national){
    ...
    zip <- system.file("extdata", "fredi_data_national.zip", package="FrEDI")
    zip_time <-tail(file.info(zip)$ctime)
    
    sys_db <- system.file("extdata", "fredi_data_national", package="FrEDI")
    ...
    if(unzip_check | time_check){
      ...
      fredi_db <- system.file("extdata","data","fredi","fredi_data_national", package="FrEDI")
