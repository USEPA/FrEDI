...
```{r new_sectors}
# c_sectorsList <- "CIL Extreme Temperature"
...
frediPath %>% load_all
df_defaults <- FrEDI::run_fredi(sectorList = c_sectorsList, aggLevels="all") %>% filter(year %in% seq(2010, 2090, by=5))
...
df_test <- run_fredi(sectorList = c_sectorsList, aggLevels="none")
df_test %>% 
  filter(year %in% seq(2010, 2090, by=5)) %>% 
  write.csv(file = file.path(saveOutputsPath, paste0(today, "_qc_outputs.csv")), row.names=F, na="")
```
