...
```{r check outputs}
test_systemData$list_impactFunctions$AirQuality_2011_NA_O3_gcm_CCSM4_Midwest(-1)
test_systemData$list_impactFunctions$AirQuality_2011_NA_O3_gcm_CCSM4_Midwest(10)
# test_systemData$slr_cm %>% filter(year > 2090) %>% arrange(model, year) %>% head
# test_systemData$slrImpacts %>% filter(year > 2090) %>% arrange(sector, variant, region, model, year) %>% head
...
c_sectorsList <- c("ATS Extreme Temperature")
```
...
frediPath %>% load_all
test1 <- list(tempInput = data.frame( year = 2000:2300, temp_C = 10)) %>% run_fredi(sectorList = c_sectorsList, aggLevels="none")
test1 %>% filter(is.infinite(annual_impacts)) %>% select(annual_impacts) %>% as.vector %>% range(na.rm=T)
test1 %>% filter(is.na(annual_impacts)) %>% select(annual_impacts) %>% as.vector %>% range(na.rm=T)
...
test2 <- list(tempInput = data.frame(year = 2000:2020, temp_C = 1) %>% rbind(data.frame(year=2021:2300, temp_C = -1))) %>% run_fredi(sectorList = c_sectorsList, aggLevels="none")
test2 %>% filter(is.infinite(annual_impacts)) %>% select(annual_impacts) %>% as.vector %>% range(na.rm=T)
test2 %>% filter(is.na(annual_impacts)) %>% select(annual_impacts) %>% as.vector %>% range(na.rm=T)
