...
```{r new_sectors}
new_sectors <- "CIL Extreme Temperature"
```
...
df_defaults %>% mutate(annual_impacts=annual_impacts %>% format(digits=15)) %>% write.csv(file = file.path(saveOutputsPath, paste0(today, "_qc_outputs.csv")), row.names=F)
```
