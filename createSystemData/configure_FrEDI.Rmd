...
frediPath %>% load_all
test1 <- run_fredi(inputsList = list(
  tempInput = data.frame( year = 2000:2300, temp_C = 4)
  # tempInput = data.frame(year = 2000:2020, temp_C = 1) %>% rbind(data.frame(year=2021:2300, temp_C = -1))
  ), sectorList = c_sectorsList, aggLevels="none")
test1 %>% filter(driverValue <0) %>% filter(model == "GISS-E2-R")  %>% select(annual_impacts) %>% as.vector %>% range(na.rm=T)
test1 %>% filter(driverValue >3) %>% filter(model == "GISS-E2-R")  %>% select(annual_impacts) %>% as.vector %>% range(na.rm=T)
test1 %>% filter(driverValue <0)  %>% select(annual_impacts) %>% as.vector %>% range(na.rm=T)
test1 %>% filter(is.infinite(annual_impacts)) %>% as.vector %>% range(na.rm=T)
```
