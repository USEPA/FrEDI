...
### Write out workbook with base test and configuration information
...
test_reshapeData <- list_reshapeData %>% dataInfo_test(
  outPath = getwd(),
  csvName = "reshapeData_testResults",
  save    = save_test,
  return  = return_test
)
```
...
test_systemData0 <- list_systemData0 %>% dataInfo_test(
  outPath = getwd(),
  csvName = "systemData0_infoResults",
  save    = save_test,
  return  = return_test
)
```
```{r initial test workbook}
######### THIS NEEDS TO BE FUNCTIONALIZED
# put initial tests into their own workbook
init_test_wb <- createWorkbook()
 addWorksheet(init_test_wb, sheetName = "reshape_data_base_test")
 addWorksheet(init_test_wb,sheetName = "createsystemData_base_test")
 
 writeDataTable(init_test_wb,sheet = "reshape_data_base_test",test_reshapeData)
 writeDataTable(init_test_wb,sheet = "createsystemData_base_test",test_systemData0)
  
 ## Get base config Data
 default_config <- tibble(
    Param_name = c("minYear","maxYear","discount_rate"),
    val = c(fredi_config$minYear,fredi_config$maxYear,fredi_config$discountRate0)
  )
 
 addWorksheet(init_test_wb,sheetName = "base_params")
 writeDataTable(init_test_wb,sheet = "base_params",default_config)
 
 ## plot GDP and Population
 addWorksheet(init_test_wb, sheetName = "default_plots")
 
 gdp_plot <- ggplot(list_systemData0$co_defaultScenario, aes(x = year, y = gdp_usd, color = region)) +
             geom_line()
   print(gdp_plot)
 insertPlot(init_test_wb,  "default_plots", width = 6, height = 4.5, fileType = "png", units = "in")

   
 pop_plot <- ggplot(list_systemData0$co_defaultScenario, aes(x = year, y = reg_pop, color = region)) +
             geom_line()
 print(pop_plot)
 insertPlot(init_test_wb,  "default_plots", xy = c("K", 2), width = 16, height = 10, fileType = "png", units = "cm")

 
 temp_plot <- ggplot(list_systemData0$co_defaultTemps, aes(x = year, y = temp_C_global)) +
             geom_line()
 print(temp_plot)
 insertPlot(init_test_wb,  "default_plots", xy = c("A", 25), width = 16, height = 10, fileType = "png", units = "cm")
 
 ## Save Workbook
 saveWorkbook(init_test_wb, "base_test_results.xlsx",overwrite=TRUE)
...
(remove objects )
```{r test_systemData0}
test_systemData0 <- list_systemData0 %>% 
