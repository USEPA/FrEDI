...
### list_loadData
list_loadData <- loadData(excelName = excelFileName, silent=T)
```
...
(remove objects )
```{r test_systemData0}
test_systemData0 <- list_systemData0 %>% configTest_newSectors(
  oldDataFile = projectPath %>% file.dir("createSystemData", "data", "sysdata.rda"),
  # oldDataFile = projectPath %>% file.dir("createSystemData", "data", "dataLog", basename(configArchName)),
  csvName = "configTest_newSectors_results.csv",
  save    = T,
  return  = T,
)
```
...
```{r test_scalars}
list_systemData$slrImpacts$scaled_impacts %>% range(na.rm=T)
### test_scalars: dataframe with configuration values for scalars 
test_scalars <- list_systemData$scalarDataframe
test_scalars %>% glimpse
### test_mainScalars: dataframe with annual values for scalars 
test_mainScalars <- list_systemData$df_mainScalars
...
test_slr_cm <- list_systemData$slr_cm %>% 
...
```{r test_imp_cm}
test_imp_cm <- list_systemData$slrImpacts %>% 
  mutate(model_fact = model_dot %>% fun_slrModel2Height(include="values", valType="factor",labelType="character"))
test_imp_cm %>% glimpse
...
test_ext_cm <- fun_slrConfigExtremes(
  slr_x = list_systemData$slr_cm,    ### rDataList$slr_cm
  imp_x = list_systemData$slrImpacts ### rDataList$slrImpacts
)
...
```{r test_ext_cm}
# frediPath %>% load_all
test_ext_cm <- list_systemData$slrExtremes #%>% mutate(model_fact = model_dot %>% fun_slrModel2Height(include="values", valType="factor",labelType="character"))
test_ext_cm %>% glimpse
test_ext_cm %>% filter(impacts_intercept < 0) %>% nrow
test_ext_cm %>% filter(impacts_slope < 0) %>% nrow
...
```{r test_co_regions}
test_co_regions <- list_systemData$co_regions; test_co_regions
```
...
```{r}
list_systemData$co_models %>% glimpse
list_systemData$co_modelTypes %>% glimpse
```
...
```{r test_co_models}
test_co_models  <- list_systemData$co_models %>% select(c("model_dot", "modelType")); test_co_models %>% glimpse
test_models_dot <- (test_co_models %>% filter(modelType %in% c("gcm")))$model_dot; test_models_dot
test_gcm_impact <- list_systemData$list_impactFunctions[1:1e3]; test_gcm_impact %>% head %>% names
...
### Uncomment following two lines to create and save data and check the outputs
time1           <- Sys.time()
list_systemData <- createSystemData(save=T, excelName = excelFileName, outPath = sysArchName)
list_systemData <- createSystemData(save=T, excelName = excelFileName, silent=T)
time2           <- Sys.time()
time_create     <- time2 - time1; time_create
```
