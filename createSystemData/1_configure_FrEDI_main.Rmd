...
```{r test createSVData}
# ## Uncomment following five lines to create and save data and check the outputs
...
```{r update_sysdata}
for(name_i in codeNames){ source(paste(codePath, name_i, sep="/")) }
update_sysdata(sv=T, impacts=T, save=T)
```
...
test1 <- testInputs$oth %>% run_fredi(sectorList = c_sectorsList, aggLevels="all")
...
```{r test sv}
frediPath %>% load_all
saveOutputsPath <- frediPath %>% file.path("..", "analysis", "qc", "20220913")
test_sv <- run_fredi_sv(sector="Coastal Properties", save = T, outpath = saveOutputsPath, addDate = T)
