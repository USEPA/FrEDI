...
```{r}
test_newSectors_config$co_defaultTemps %>% identical(list_systemData0$co_defaultTemps)
dim(test_newSectors_config$co_defaultTemps) %>% c(dim(list_systemData0$co_defaultTemps))
test_newSectors_config$gdp_default %>% identical(list_systemData0$gdp_default)
dim(test_newSectors_config$gdp_default) %>% c(dim(list_systemData0$gdp_default))
```
...
## Test Run against defualt run
```{r test FrEDI Output Against default data}
