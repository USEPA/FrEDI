...
c_sv_sectors     <- get_sv_sectorInfo()
...
# resultsPath <- "C:/Users/wmaddock/Documents/GitHub/FrEDI/createSystemData"
```
...
test_sv <-
  ...
  (function(
    ...
    results_x <- sectors %>% lapply(function(sector_i, inputs_i=inputs_x){
      ...
      sysTime_x <- Sys.time()
      ### Get inputs
      type_i  <- ifelse(sector_i %in% c_slr, "slrInput", "tempInput")
      col_i   <- ifelse(sector_i %in% c_slr, "slr_cm"  , "temp_C")
      ### Create drivers if not null
      inputs_i <- NULL
      if(!is.null(inputs)){
        ### Scenarios
        c_scenarios_i <- inputs %>% names
        inputs_i      <- c_scenarios_i %>% lapply(function(scen_j, dataList = inputs, type_j=type_i){
          paste0(scen_j, ": ", type_j) %>% print
          list_j   <- dataList[[scen_j]]
          inputs_j <- list_j[[type_j]]
          inputs_j <- inputs_j %>% mutate(scenario = scen_j)
          inputs_j <- inputs_j %>% select(c("year", all_of(col_i), "scenario"))
          # inputs_j %>% glimpse
          return(inputs_j)
        }) %>% (function(scen_j){do.call(rbind, scen_j)})
      } ### End if(!is.null(inputs))
      paste0("\t", "Input names: ", paste(inputs_i %>% names, collapse=", "), "...") %>% message()
      c_check_i <- inputs_i$scenario %>% unique; inputs_i %>% glimpse
      check_i   <- all(c_check_i %in% c_scenarios_i) & all(c_scenarios_i %in% c_check_i)
