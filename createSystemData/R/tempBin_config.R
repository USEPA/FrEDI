...
tempBinData_config <- list()
###### Messages for createSystemData ######
tempBinData_config$messages_data = list(
...
### Millions, billions
tempBinData_config$k_million <- 10^6
tempBinData_config$k_billion <- 10^9

###### temps2slr data info ######
# tempBinData_config$temps2slr <- list(
...
# eqYear        = 1849)
# 
# tempBinData_config$temps2slr$sample_fNames <- "." %>% 
#   paste(tempBinData_config$temps2slr$rel_cmip, "sample_outputstream", sep="/") %>%
#   paste(tempBinData_config$temps2slr$rcpList, sep="_") %>% 
#   paste("csv", sep=".")

###### Data years and eras ######
### Min year, max year
tempBinData_config$minYear <- 2010
tempBinData_config$maxYear <- 2090
...
tempBin_config <- tempBinData_config
###### List of Messages ######
tempBin_config$list_messages = list(
    loadPackages  = "Loading required packages...",
    sourceCode    = "Loading custom functions...",
    loadRData     = list(
      try    ="Loading R data...",
...
tempBin_config$list_years_by20 <- seq(tempBin_config$minYear, tempBin_config$maxYear, 20)
tempBin_config$list_years_by10 <- seq(tempBin_config$minYear, tempBin_config$maxYear, 10) 
tempBin_config$list_years_by5  <- seq(tempBin_config$minYear, tempBin_config$maxYear, 5)  
tempBin_config$list_years      <- seq(tempBin_config$minYear, tempBin_config$maxYear, 1)
###### Default values ######
### Base year, discount rate
### Types of aggregation
tempBin_config$baseYear0     = 2010
tempBin_config$rate0         = 0.3
tempBin_config$discountRate0 = 0.03
tempBin_config$aggList0      = c("national", "model", "impactYear", "impactType")
tempBin_config$groupLevels0  = c("sector", "adaptation", "impactYear", "impactType", "modelType", "model", "region")
  
###### temps2slr constants ######
tempBin_config$temps2slr = list(
