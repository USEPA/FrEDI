...
loadData <- function(
  ...
  for (i in 1:num_dataTables) {
    ...
    excludeCols_i <- tableInfo_i$excludeCol_ids %>%
      str_split(", ") %>%
      unlist() %>%
      as.numeric()
...
reshape_loadData <- function(
  ...
  c_impactYearLevels <- co_impactYears %>%
    select(-c(all_of(drop0))) %>%
    names()
  ...
  co_impactYears <- co_impactYears %>%
    filter(!is.na(impactYear2)) %>%
    select(-c(all_of(drop1)))
  ...
  ### Mutate model
  c_regSlr <- slrImpacts[[select0]] %>%
    str_split("_") %>%
    (function(i) {
      do.call(rbind, i)
    }) %>%
    as_tibble()
  c_regSlr <- c_regSlr %>% (function(x) {
    colnames(x) <- names0
    return(x)
  })
  ...
  ### Check impacts
  "nrow(c_regSlr) == nrow(slrImpacts)" %>%
    paste0(": ", nrow(c_regSlr) == nrow(slrImpacts)) %>%
    print()
  ...
  list_scaledImpacts <- list_scaledImpacts %>%
    ...
    lapply(function(name_i, list_x = list_scaledImpacts) {
      ...
      df_i <- df_i %>%
        mutate(impactType = sector_impactType) %>%
        select(-c(all_of(select0)))
      rm("levels0", "labels0", "select0")
      ...
      if (doDot0) {
        df_i <- df_i %>% mutate(model_dot = model %>% factor(levels0, labels0))
      } else {
        df_i <- df_i %>% mutate(model_dot = model)
      }
    ...
    (function(list_x, names_x = names(list_scaledImpacts)) {
      names(list_x) <- names_x
      return(list_x)
    })

  ### Update objects in environment
  for (name_i in names(list_scaledImpacts)) {
    assign(name_i, list_scaledImpacts[[name_i]])
  }
