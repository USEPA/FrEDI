...
test_loadData <- function(
  ...
  check_na <- function(x){
    tibble(n_na = colSums(is.na(x)),
           n_rows = nrow(x),
           na_all = n_rows>n_na) %>% filter(na_all == FALSE) %>% nrow()
  }
  ...
  test_tab <- dat_att %>% 
    mutate(
      ...
      dup_vals = case_when( row_count == unique_rows ~ FALSE,
                            Table.Name == "data_scaledImpacts" ~ FALSE,
                            
                            ),
      miss_val_check = case_when(miss_val_check == 0 ~ FALSE,
  ...
  if_else(
    ...
    true = {
      ...
      ### change the repetitive oudirectory defintino ot a outdir
      ### clean up the paste0 filepaths
