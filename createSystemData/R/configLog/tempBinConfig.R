...
tempBinConfig <- function(outPath){
  # function(...){
  ...
  ###### List of Messages for tempBin ######
  messages_tempBin <- list(
  ...
  tempBin_config     <- list(
    ...
    messages_tempBin = messages_tempBin,
  ...
  #   messages_data    = messages_data,
  #   messages_tempBin = messages_tempBin,
  #   tempBin_config   = tempBin_config
  # )
  # ### Save the list
  # configFilePath  <- paste(system.file(package="ciraTempBin"), "extdata", "tempBin_config.rda", sep="/")
  save(tempBin_config, file=outPath)
 # return(returnList)
}
### Run the function
# tempBinConfig()
