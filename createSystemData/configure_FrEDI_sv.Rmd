...
test_svData     <- createSVData(save=F, sv = F, pop=F, impacts = T, impactSectors = c("Coastal Properties"), format=F)
time2           <- Sys.time()
...
```{r}
# for(name_i in names(test_svData$svDataList$svData)){
#   paste0(name_i, ": ", paste(test_svData$svDataList$svData[,name_i] %>% class, collapse=", ")) %>% print
# }
```
