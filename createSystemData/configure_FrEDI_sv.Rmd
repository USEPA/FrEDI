...
test_svData     <- createSVData(save=T, sv = F, pop=F, impacts = T, impactSectors = c("Coastal Properties"), format=F)
# test_svData     <- createSVData(save=T, sv = T, pop=T, impacts = T, impactSectors = c("Coastal Properties"), format=F)
time2           <- Sys.time()
...
load(file.path(frediPath, "R", "svDataList.rda"))
load(file.path(frediPath, "R", "svPopData.rda"))
load(file.path(frediPath, "data", "sv", "impactsList_coastalProp_withAdapt.rda"))
...
Look at `svDataCoastal` (17,824 rows, 22 columns):
...
```{r svDataCoastal}
### Standardize names
# svDataCoastal <- svDataList$svDataCoastal %>%
...
```{r}
svDataCoastal$region %>% unique
test_sv_fips  <- svDataCoastal$fips     %>% unique; test_sv_fips  %>% (function(x){paste0(length(x), ": ", paste(head(x), collapse=", "))})
test_sv_geos  <- svDataCoastal$geoid10  %>% unique; test_sv_geos  %>% (function(x){paste0(length(x), ": ", paste(head(x), collapse=", "))})
# svDataCoastal$nChar %>% range %>% c(svDataCoastal$nChar2 %>% range)
# test_sv_geos2 <- svDataCoastal$geoid102 %>% unique; test_sv_geos2 %>% (function(x){paste0(length(x), ": ", paste(head(x), collapse=", "))})
```
...
```{r popCoastal}
popCoastal <- svDataList$svDataCoastal %>% 
  # select(-c("county_pop")) %>%
  left_join(test_pop, by = c("region", "state", "geoid10"))
popCoastal %>% glimpse
