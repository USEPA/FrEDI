...
interpolate_annual <- function(
  ...
  if (byState) {defCols <- defCols |> c("state", "postal")}
  ...
  #### States
  if (byState) {states0 <- data |> get_uniqueValues("state"); postals0 <- data |> get_uniqueValues("postal")}
  ...
  ### Drop yCol from data
  data   <- data |> select(-c(all_of(column0)))
