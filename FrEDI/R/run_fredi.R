...
#'
#' @description
#' This function allows users to project annual average climate change impacts through 2300 (2010-2300) for available sectors (see [FrEDI::get_sectorInfo()]). Users may specify an optional list of custom scenarios. The output is an R data frame object containing annual average impacts, by year, for each sector, variant, impact type, model (GCM or SLR scenario), and region.
#'
...
#' `impactyear` \tab Annual results for sectors with only one impact year estimate (i.e., `impactYear = "N/A"`) are separated from those with multiple impact year estimates. Sectors with multiple impact years have separate results for impact years 2010 and 2090. For these sectors, annual results are interpolated between impact year estimates in the following way: data is grouped by columns `"sector", "variant", "impactType, "model_type", "model", "region", "year"`. Results for model run years between 2010 and 2090 are interpolated across impact year estimates by adjusting the difference between the two estimates by a factor that accounts for the proximity of the impact years to the model run year and adding the adjusted difference to the lower estimate. For any model run years above 2090, annual results for sectors with multiple impact years return the 2090 estimate. The interpolated values are bound back to the results for sectors with a single impact year estimate, and column `impactYear` set to `impactYear="Interpolation"` for all values. \cr
#' }
#'
