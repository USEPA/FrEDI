...
The **Framework for Evaluating Damages and Impacts (FrEDI)** was developed to further understand and communicate the potential physical and economic impacts of future climate change in the United States, under any custom emission or temperature scenario.

FrEDI works by first taking user-defined projections of U.S. population, gross domestic product (GDP), and mean surface temperature. When run, FrEDI and then draws upon underlying region and sector-specific temperature-impact relationships to project and output the annual physical and economic impacts of climate change across multiple impact sectors, U.S. regions, and populations within the contiguous U.S. (CONUS), through the end of the 21st century. 
...
**FrEDI draws upon a wide range of existing scientific literature.** FrEDI currently draws upon over 30 existing peer-reviewed studies and climate change impact models and uses an impacts-by-degree temperature binning approach [(Sarofim et al., 2021)](https://link.springer.com/article/10.1007/s10584-021-03048-6), to estimate the relationship between future degrees of warming and impacts across more than 20 impact category sectors. Many of these sectoral studies have been adapted from EPA’s [Climate Change Impacts and Risk Assessment (CIRA)](https://www.epa.gov/cira) project, which began in 2008 to assess and compare the impacts of climate change in the U.S. using a consistent set of climate models and socioeconomic scenarios. External studies are now also integrated into FrEDI in cases where the underlying studies can provide regional physical or economic impacts by degree of warming (or cm of sea level rise) and can be scaled to account for future socioeconomic (e.g., GDP or population) or sector-specific (e.g., miles of roadways) conditions. 
...
A CSV containing temperatures for the contiguous U.S. (CONUS) in degrees Celsius relative to the 1986-2005 average (degrees of warming relative to the baseline). Temperature values must be greater than or equal to zero Celsius. The first column needs to contain years in the interval 2000 to 2100 and second column contains temperatures, in degrees Celsius, above the 1995 baseline. Users can convert global temperatures to CONUS temperatures using `FrEDI::convertTemps(from="global")` or by specifying `FrEDI::import_inputs(temptype="global")` when importing a temperature scenario from a CSV file.
...
A CSV file containing a custom U.S. population scenario for FrEDI's seven CONUS regions. The first column contains years in the interval 2000 to 2100. The number of additional columns, column names, and column contents depend on the population format set by `popform`. For more details, see `?popform`.

### U.S. GDP
A CSV file containing a custom scenario for U.S. gross domestic product (GDP). The first column contains years in the interval 2000 to 2100 and the second column contains values for GDP, in total 2015$.
